...
const (
	...
	ErrCodeGetDataSourceAuth                                      // 查询数据源授权失败
)

// 可信任务管理错误码: 500~599
const (
	ErrCodeAddCalcTask                          ErrCode = iota + 500 // 创建可信任务失败
	ErrCodeUpdateCalcTask                                            // 更新可信任务失败
	ErrCodeGetCalcTask                                               // 从链上获取可信任务详情失败
	ErrCodeGetCalcTaskList                                           // 从链上获取可信任务列表信息失败
	ErrCodeAuthorizeCalcTaskEvent                                    // 创建授权可信任务邀请事件失败
	ErrCodeRunSubmitCmd                                              // 可信任务提交失败
	ErrCodeAddCalcTaskInstance                                       // 创建可信任务实例失败
	ErrCodeCheckResultDB                                             // 计算结果接收DB探测失败
	ErrCodeGetCalcTaskInstanceListByNoAndStatus                      // 根据任务编号获取可信任务实例列表失败
	ErrCodeCalcTaskInstanceAlreadyRunning                            // 可信任务实例已经启动
	ErrCodeCalcTaskInstanceAlreadyCreated                            // 可信任务实例已经创建
	ErrCodeNoAnyCalcTask                                             // 该成员未创建任何可信任务
	ErrCodeCreateCalcTaskInfo                                        // 创建可信任务包体失败
	ErrCodeCreateTaskRequest                                         // 创建可信任务请求失败
	ErrCodeAddCalcTaskRequest                                        // 可信计算任务请求上链失败
	ErrCodeAddDataSharingTaskRequest                                 // 可信共享任务请求上链失败
	ErrCodeCreateAuthorizeCalcTaskEvent                              // 创建可信任务授权事件失败
	ErrCodeCreateAuthorizeDataSharingTaskEvent                       // 创建可信共享任务授权事件失败
	ErrCodeCalcTaskNotExists                                         // 可信任务不存在
	ErrCodeCalcTaskExists                                            // 可信任务已存在
	ErrCodeGetRunningCalcTaskInstanceCount                           // 获取计算资源运行中的任务实例数量失败
	ErrCodeStopCalcTask                                              // 终止可信任务失败
	ErrCodeStopCalcNoAuth                                            // 无权终止可信任务
	ErrCodeCalcTaskUnauth                                            // 可信任务未授权完成
	ErrCodeCheckConsMemberValid                                      // 检查可信任务参与方有效性失败
	ErrCodeCalcTaskInvalidCreator                                    // 无效可信任务创建方
	...
	ErrCodeEncryptTask                                               // 任务敏感信息加密失败
	ErrCodeGetCalcTaskRequestByNo                                    // 根据任务编号从链上获取可信计算任务请求体失败
	ErrGetDataSharingTaskRequestByNo                                 // 根据任务编号从链上获取可信共享任务请求体失败
)
...
const (
	...
	ErrCodeGetTeeResourceIdByNo                                // 根据可信计算资源编号查ID失败
	...
	ErrCodeGetChainConfIdByNo                                  // 根据链配置编号查ID失败
	...
	ErrCodeGetChainTxInfo                                      // 获取链交易详情信息失败
...
// 共享代理相关错误码: 900+
const (
	ErrCodeAddDataSharingProxy                ErrCode = iota + 900 // 新增共享代理失败
	ErrCodeGetDataSharingProxyByNo                                 // 根据共享代理编号获取详情失败
	ErrCodeGetDataSharingProxyList                                 // 获取共享代理列表失败
	ErrCodeGetDataSharingProxyNameList                             // 获取当前管控台共享代理名称列表失败
	ErrCodeDataSharingProxyNotExists                               // 共享代理不存在
	ErrCodeDataSharingProxyExists                                  // 共享代理已存在
	ErrCodeDataSharingProxySameName                                // 共享代理名称重复
	ErrCodeCheckDataSharingProxyExists                             // 检查共享代理是否存在失败
	ErrCodeCheckDataSharingProxyExistsByAppId                      // 检查共享代理是否归属指定参与方失败
	ErrCodeDataSharingProxyNameExists                              // 共享代理网关名称已存在
	ErrCodeUpdateDataSharingProxy                                  // 更新共享代理网关信息失败
	ErrCodeUpdateDataSharingProxyStatus                            // 更新共享代理网关状态失败
	ErrCodeUnknownDataSharingProxyStatus                           // 未知共享代理状态
	ErrCodeInsertDataSharingProxyCertKeyInfo                       // 共享代理网关证书和私钥入库失败
	ErrCodeGetDataSharingProxyNoListByAppId                        // 获取指定参与方共享代理编号列表失败
	ErrCodeMkdirDataSharingProxy                                   // 创建共享代理网关配置目录失败
	ErrCodeGetDataSharingProxyCertKeyInfoByNo                      // 获取共享代理网关链身份私钥信息失败
	ErrCodeWriteFileDataSharingProxy                               // 写入共享代理网关配置文件失败
	ErrCodeParseConfigTemplate                                     // 渲染共享代理配置模板失败
	ErrCodeZipDataSharingProxy                                     // 打包共享代理下载安装包失败
	ErrCodeReadZipDataSharingProxy                                 // 本地读取打包好的共享代理安装包文件失败
	ErrCodeGetSubscribedBlockHeight                                // 获取已经订阅的区块高度失败
)
...
const (
	...
	ErrCodeApplyChainAccessService
)

// 账户相关错误码：2000+
const (
	ErrCodeInvalidUserName           ErrCode = iota + 2000 //登录账号格式错误
...
var ErrMessage = map[ErrCode][]string{
	...
	// 可信任务管理错误码: 500~599
	ErrCodeAddCalcTask:                          {"FailedOperation.AddCalcTask", "add calc task failed", "创建可信任务失败"},
	ErrCodeUpdateCalcTask:                       {"FailedOperation.UpdateCalcTask", "update calc task failed", "更新可信任务失败"},
	ErrCodeGetCalcTask:                          {"FailedOperation.GetCalcTask", "get calc task failed", "从链上获取可信任务详情失败"},
	ErrCodeGetCalcTaskList:                      {"FailedOperation.GetCalcTaskList", "get calc task list failed", "从链上获取可信任务列表信息失败"},
	ErrCodeAuthorizeCalcTaskEvent:               {"FailedOperation.AuthorizeCalcTaskEvent", "authorize calc task event failed", "创建授权可信任务邀请事件失败"},
	ErrCodeRunSubmitCmd:                         {"FailedOperation.RunSubmitCmd", "run submit cmd failed", "可信任务提交失败"},
	ErrCodeAddCalcTaskInstance:                  {"FailedOperation.AddCalcTaskInstance", "add calc task instance failed", "创建可信任务实例失败"},
	ErrCodeCheckResultDB:                        {"FailedOperation.CheckResultDB", "check result db failed", "计算结果接收DB探测失败"},
	ErrCodeGetCalcTaskInstanceListByNoAndStatus: {"FailedOperation.GetCalcTaskInstanceListByNoAndStatus", "get calc task instance list by no and status failed", "根据任务编号获取可信任务实例列表失败"},
	ErrCodeCalcTaskInstanceAlreadyRunning:       {"FailedOperation.CalcTaskInstanceAlreadyRunning", "calc task instance already running", "可信任务实例已经启动"},
	ErrCodeCalcTaskInstanceAlreadyCreated:       {"FailedOperation.CalcTaskInstanceAlreadyCreated", "calc task instance already created", "可信任务实例已经创建"},
	ErrCodeNoAnyCalcTask:                        {"FailedOperation.NoAnyCalcTask", "no any calc task", "该成员未创建任何可信任务"},
	ErrCodeCreateCalcTaskInfo:                   {"FailedOperation.CreateCalcTaskInfo", "create calc task info failed", "创建可信任务包体失败"},
	ErrCodeCreateTaskRequest:                    {"FailedOperation.CreateTaskRequest", "create calc task request failed", "创建可信任务请求失败"},
	ErrCodeAddCalcTaskRequest:                   {"FailedOperation.AddCalcTaskRequest", "add calc task request on chain failed", "可信计算任务请求上链失败"},
	ErrCodeAddDataSharingTaskRequest:            {"FailedOperation.AddDataSharingTaskRequest", "add data sharing task request on chain failed", "可信共享任务请求上链失败"},
	ErrCodeCreateAuthorizeCalcTaskEvent:         {"FailedOperation.CreateAuthorizeCalcTaskEvent", "create authorize calc task event failed", "创建可信任务授权事件失败"},
	ErrCodeCreateAuthorizeDataSharingTaskEvent:  {"FailedOperation.CreateAuthorizeDataSharingTaskEvent", "create authorize data sharing task event failed", "创建可信共享任务授权事件失败"},
	ErrCodeCalcTaskNotExists:                    {"FailedOperation.CalcTaskNotExists", "calc task not exists", "可信任务不存在"},
	ErrCodeCalcTaskExists:                       {"FailedOperation.CalcTaskExists", "calc task exists", "可信任务已存在"},
	...
	ErrCodeStopCalcTask:                         {"FailedOperation.StopCalcTask", "stop calc task failed", "终止任务失败"},
	ErrCodeStopCalcNoAuth:                       {"FailedOperation.StopCalcNoAuth", "no auth to stop task", "无权终止任务"},
	ErrCodeCalcTaskUnauth:                       {"FailedOperation.CalcTaskUnauth", "calc task unauth", "可信任务未授权完成"},
	ErrCodeCheckConsMemberValid:                 {"FailedOperation.CheckConsMemberValid", "check calc task member valid", "检查可信任务参与方有效性失败"},
	ErrCodeCalcTaskInvalidCreator:               {"FailedOperation.CalcTaskInvalidCreator", "calc task invalid creator", "无效可信任务创建方"},
	...
	ErrCodeCalcTaskInvalidResultUser:            {"FailedOperation.CalcTaskInvalidResultUser", "calc task invalid result user", "无效计算结果使用方"},
	ErrCodeEncryptTask:                          {"FailedOperation.EncryptTask", "encrypt task failed", "任务敏感信息加密失败"},
	ErrCodeGetCalcTaskRequestByNo:               {"FailedOperation.GetCalcTaskRequestByNo", "get calc task request by no failed", "根据任务编号从链上获取可信计算任务请求体失败"},
	ErrGetDataSharingTaskRequestByNo:            {"FailedOperation.DataSharingTaskRequestByNo", "get data sharing task request by no failed", "根据任务编号从链上获取可信共享任务请求体失败"},
	...
	ErrCodeGetTeeResourceIdByNo:           {"FailedOperation.GetTeeResourceIdByNo", "get tee resource id by no failed", "根据可信计算资源编号查ID失败"},
	...
	ErrCodeGetChainConfIdByNo:             {"FailedOperation.GetChainConfIdByNo", "get chain conf id by no failed", "根据链配置编号查ID失败"},
	...
	ErrCodeGetChainTxInfo:                 {"FailedOperation.GetChainTxInfo", "get chain tx info failed", "获取链交易详情信息失败"},
	...
	// 共享代理相关: 900+
	ErrCodeAddDataSharingProxy:                {"FailedOperation.AddDataSharingProxy", "add data sharing proxy failed", "新增共享代理失败"},
	ErrCodeGetDataSharingProxyByNo:            {"FailedOperation.GetDataSharingProxyByNo", "get data sharing proxy info by no failed", "根据共享代理编号获取详情失败"},
	ErrCodeGetDataSharingProxyList:            {"FailedOperation.GetDataSharingProxyList", "get data sharing proxy list failed", "获取共享代理列表失败"},
	ErrCodeDataSharingProxyNotExists:          {"FailedOperation.DataSharingProxyNotExists", "data sharing proxy not exists", "共享代理不存在"},
	ErrCodeDataSharingProxyExists:             {"FailedOperation.DataSharingProxyExists", "data sharing proxy already exists", "共享代理已存在"},
	ErrCodeDataSharingProxySameName:           {"FailedOperation.ErrCodeDataSharingProxySameName", "data sharing proxy name same", "共享代理名称重复"},
	ErrCodeCheckDataSharingProxyExists:        {"FailedOperation.CheckDataSharingProxyExists", "check if data sharing proxy exists failed", "检查共享代理是否存在失败"},
	ErrCodeCheckDataSharingProxyExistsByAppId: {"FailedOperation.CheckDataSharingProxyExistsByAppId", "check if data sharing proxy exists by appId failed", "检查共享代理是否归属指定参与方失败"},
	ErrCodeGetDataSharingProxyNameList:        {"FailedOperation.GetDataSharingProxyNameList", "get data sharing proxy name list failed", "获取当前管控台共享代理名称列表失败"},
	ErrCodeDataSharingProxyNameExists:         {"FailedOperation.DataSharingProxyNameExists", "data sharing proxy name already exists", "共享代理网关名称已存在"},
	ErrCodeUpdateDataSharingProxy:             {"FailedOperation.UpdateDataSharingProxy", "update data sharing proxy failed", "更新共享代理网关信息失败"},
	ErrCodeUpdateDataSharingProxyStatus:       {"FailedOperation.UpdateDataSharingProxyStatus", "update data sharing proxy status failed", "更新共享代理网关状态失败"},
	ErrCodeInsertDataSharingProxyCertKeyInfo:  {"FailedOperation.InsertDataSharingProxyCertKeyInfo", "insert data sharing proxy cert key info to db failed", "共享代理网关证书和私钥入库失败"},
	ErrCodeGetDataSharingProxyNoListByAppId:   {"FailedOperation.GetDataSharingProxyNoListByAppId", "get data sharing proxy no list by appId failed", "获取指定参与方共享代理编号列表失败"},
	ErrCodeMkdirDataSharingProxy:              {"FailedOperation.MkdirDataSharingProxy", "mkdir data sharing proxy failed", "创建共享代理网关配置目录失败"},
	ErrCodeGetDataSharingProxyCertKeyInfoByNo: {"FailedOperation.GetDataSharingProxyCertKeyInfoByNo", "get data sharing proxy cert key info by no failed", "获取共享代理网关链身份私钥信息失败"},
	ErrCodeWriteFileDataSharingProxy:          {"FailedOperation.WriteFileDataSharingProxy", "write data sharing proxy config failed", "写入共享代理网关配置文件失败"},
	ErrCodeParseConfigTemplate:                {"FailedOperation.ParseConfigTemplate", "parse config template failed", "渲染共享代理配置模板失败"},
	ErrCodeZipDataSharingProxy:                {"FailedOperation.ZipDataSharingProxy", "zip data sharing proxy pkg failed", "打包共享代理安装包文件失败"},
	ErrCodeReadZipDataSharingProxy:            {"FailedOperation.ReadZipDataSharingProxy", "read zip data sharing proxy pkg failed", "本地读取打包好的共享代理安装包文件失败"},
	ErrCodeGetSubscribedBlockHeight:           {"FailedOperation.GetSubscribedBlockHeight", "get subscribed block height failed", "获取已经订阅的区块高度失败"},
