...
func TestSM3(t *testing.T) {
	Convey("test sm3", t, func() {
		ctxSize := base.SM3CtxSize()
		fmt.Printf("SM3CtxSize is %d\n", ctxSize)
		So(ctxSize, ShouldNotBeZeroValue)
		var sm3context base.SM3_ctx_t
		base.SM3Init(&sm3context)
		data := []byte("helloworld")
		base.SM3Update(&sm3context, data[:], len(data))
		var out [32]byte
		base.SM3Final(&sm3context, out[:])
		fmt.Printf("%x", out[:])
	})
}
